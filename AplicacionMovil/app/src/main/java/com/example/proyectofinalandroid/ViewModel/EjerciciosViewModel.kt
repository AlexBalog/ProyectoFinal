...
class EjerciciosViewModel @Inject constructor(private val repository: EjerciciosRepository) : ViewModel() {
    ...
    fun getAllEjercicios() {
        viewModelScope.launch {
            try {
                val lista = repository.getAllEjercicios(token = _usuario.value?.token.toString())
                if (lista != null) {
    ...
    fun getOneEjercicio(id: String) {
        ...
        viewModelScope.launch {
            _ejercicioSeleccionado.value = repository.getOneEjercicio(id, _usuario.value?.token.toString())
        }
    }

    fun getFilterEjercicios(filtros: Map<String, String>) {
        viewModelScope.launch {
            try {
                val lista = repository.getFilterEjercicios(_usuario.value?.token.toString(), filtros)
                if (lista != null) {
                    _ejercicios.value = lista
